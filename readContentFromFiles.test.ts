...
Deno.test({
    ...
    fn: async () => {
        ...
        assertEquals(contents, `POST https://mock-api.deno.dev?body=Pong
Accept: text/plain
...
Deno.test({
    ...
    fn: async () => {
        ...
        for await (const content of readContentFromFiles([Deno.cwd() + '/http/ping-pong.http'])) {
            contents += content;
        }
        assertEquals(contents, `POST https://mock-api.deno.dev?body=Pong
Accept: text/plain
...
`
        ...
        for await (const content of readContentFromFiles(['http/ping-pong.http', 'http/ping-pong.http'])) {
            contents += content;
        }
