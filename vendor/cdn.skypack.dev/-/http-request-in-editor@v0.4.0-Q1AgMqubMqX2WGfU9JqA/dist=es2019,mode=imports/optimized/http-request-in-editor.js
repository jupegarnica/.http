...
const {isMethod: isMethod$1, isRequestTarget: isRequestTarget$1, isHttpVersion: isHttpVersion$1} = predicates;
...
const EnvVariable = stampit2(valueNode, {
  ...
  methods: {
    get name() {
      return this.value.match(/^{{\s*(?<variable_name>[a-zA-Z0-9_-]+)\s*}}$/).groups.variable_name;
    }
  }
});
...
var require$$0$1 = /* @__PURE__ */ getDefaultExportFromNamespaceIfNotNamed(ramdaAdjunct);
...
const request$1 = ([
  ...
  if (isResponseHandler$1(responseHandlerNode) && responseHandlerNode.children.length > 0) {
...
const requestLine$1 = ([methodNode, requestTargetNode, httpVersionNode], location) => {
  const children = [];
...
const method$1 = ([httpVerb], location) => cst.Method({location, value: httpVerb});
const httpVersion$1 = (data, location) => cst.HttpVersion({
  location,
  value: `${stringify(data[1])}.${stringify(data[3])}`
});
const requestTarget$1 = (data, location) => cst.RequestTarget({location, children: data[0]});
const originForm$1 = ([absolutePathNode, queryMatch, fragmentMatch], location) => {
...
const absoluteForm$1 = ([schemeMatch, hierPartNode, queryMatch, fragmentMatch], location) => {
  const children = [];
  if (schemeMatch !== null) {
...
const asteriskForm$1 = (data, location) => cst.AsteriskForm({location, value: data[0]});
const scheme$1 = (data, location) => cst.Scheme({location, value: stringify(flatten(data))});
...
const port$1 = (data, location) => cst.Port({location, value: stringifyId(data)});
...
const ipv6Address$1 = (data, location) => cst.Ipv6Address({location, value: stringifyId(data)});
...
const pathSeparator$1 = (data, location) => cst.PathSeparator({location, value: data[0]});
const segment$1 = (data, location) => cst.Segment({location, value: stringifyId(data)});
...
const headers$1 = (data, location) => cst.Headers({location, children: [...data[0]]});
const headerField$1 = (data, location) => cst.HeaderField({location, children: [data[0], data[3]]});
const fieldName$1 = (data, location) => cst.FieldName({location, value: stringifyId(data)});
...
const messages$1 = (data, location) => cst.Messages({location, children: [...data[0]]});
const messageLine$1 = (data, location, reject) => {
  const lineTail2 = stringifyId(data);
  if (lineTail2.includes("<") || lineTail2.includes("> ") || lineTail2.includes("<> ") || lineTail2.includes("###")) {
    return reject;
  }
  return cst.MessageLine({location, value: lineTail2});
};
const inputFileRef$1 = (data, location) => cst.InputFileRef({location, children: [data[2]]});
const filePath$1 = (data, location) => cst.FilePath({location, value: stringifyId(data)});
const responseHandler$1 = (data, location) => cst.ResponseHandler({location, children: [data[0]]});
...
const responseRef$1 = (data, location) => cst.ResponseRef({location, children: [data[2]]});
...
const envVariable$1 = (data, location) => cst.EnvVariable({location, value: stringify(flatten(data))});
var postprocessors = {
  nth,
  stubNull,
  stringifyId,
  ...
  (function() {
    ...
    var grammar2 = {
      ...
      ParserRules: [
        ...
        {name: "MESSAGE_BODY", symbols: ["MESSAGES"], postprocess: messageBody3},
        ...
        {name: "MESSAGES", symbols: ["MESSAGES$ebnf$1"], postprocess: messages2},
        ...
        {name: "__", symbols: ["REQUIRED_WHITESPACE"], postprocess: stubNull2},
    ...
    if (isLeaving) {
      ...
      if (isEdited) {
        if (inArray) {
          ...
          node2 = Object.create(Object.getPrototypeOf(node2), Object.getOwnPropertyDescriptors(node2));
        }
        let editOffset = 0;
...
export {AbsoluteFormVisitor$1 as AbsoluteFormVisitor, AbsolutePathVisitor$1 as AbsolutePathVisitor, AsteriskFormVisitor$1 as AsteriskFormVisitor, AxiosRunner$1 as AxiosRunner, AxiosVisitor$1 as AxiosVisitor, MessageBodyVisitor$1 as MessageBodyVisitor, OriginFormVisitor$1 as OriginFormVisitor, RequestTargetVisitor$1 as RequestTargetVisitor, src as __moduleExports, parse$3 as parse, runSpec$2 as runSpec, sexprs$2 as sexprs};
